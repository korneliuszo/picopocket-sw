;
; MBR boot code modified from Free FDISK
...
;-----------------------------------------------------------------------
	...
	mem_off		equ 0x0413		;byte  KB of conventional memory
...
read_option_rom:
  ...
  .intcall:
	stc
	int 0x13
	jnc .read_ok		; no read error occured?
	call fatal			; does not return
	db 'read error while reading rom', 0
  .read_ok:
...
.ip:
	pop ax
	add ax, .after_optionrom - .ip
	push ax

	pushf ; add makes input flags less valid
	push ax ; input ax invalid
	mov ax, 2
	jmp far [jmpaddr]
...
PADDING_BYTES   equ CODE_SIZE - 2 * DAP_PACKET_SIZE - 4 - ($ - $$)
